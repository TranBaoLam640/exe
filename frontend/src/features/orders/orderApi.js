...
export async function createPayOsPayment(orderId) {
  const response = await api.post(`/api/orders/${encodeURIComponent(orderId)}/payment/payos`, {});
  return response.data?.data;
}
...
export async function fetchCustomerShipment(orderId) {
  const response = await api.get(`/api/orders/${encodeURIComponent(orderId)}/shipment`);
  return response.data?.data;
}
...
export async function fetchCustomerInspections(orderId) {
  const response = await api.get(`/api/orders/${encodeURIComponent(orderId)}/inspections`);
  return response.data?.data || [];
}
